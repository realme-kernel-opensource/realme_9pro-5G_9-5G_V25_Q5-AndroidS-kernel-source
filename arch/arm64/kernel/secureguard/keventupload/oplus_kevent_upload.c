...
static int __init oplus_security_keventupload_init(void)
{
	...
	/*register trace hook*/
#ifdef CONFIG_OPLUS_KERNEL_SECURE_GUARD
	ret = register_trace_android_vh_secureguard_send_to_user(
		      oplus_kevent_send_to_user, NULL);
...
exit:
	...
	if (ret) {
		...
		if (oplus_security_trace_flag)
			unregister_trace_android_vh_secureguard_send_to_user(oplus_kevent_send_to_user,
					NULL);
