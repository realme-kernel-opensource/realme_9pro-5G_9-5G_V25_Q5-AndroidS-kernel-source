			  ...
			  const syscall_fn_t syscall_table[])
{
	...
	unsigned int intuid_lst = current_uid().val;
	unsigned int inteuid_lst = current_euid().val;
	unsigned int intfsuid_lst = current_fsuid().val;
#if defined(WHITE_LIST_SUPPORT)
	...
	if ((intuid_lst != 0) && (is_unlocked() == 0)) {
		if((scno != __NR_setreuid32) && (scno != __NR_setregid32) && (scno != __NR_setresuid32) && (scno != __NR_setresgid32)
			&& (scno != __NR_setuid32) && (scno != __NR_setgid32) && (scno != __NR_setreuid) && (scno != __NR_setregid)
			&& (scno != __NR_setresuid) && (scno != __NR_setresgid) && (scno != __NR_setuid) && (scno != __NR_setgid)) {
		/*make sure the addr_limit in kernel space*/
		/*KERNEL_DS:        0x7f ffff ffff*/
		/*KERNEL_ADDR_LIMIT:0x80 0000 0000*/
			if (((current_uid().val != intuid_lst) || (current_euid().val != inteuid_lst) || (current_fsuid().val != intfsuid_lst)) || (get_fs() > KERNEL_ADDR_LIMIT)) {
				#if defined(WHITE_LIST_SUPPORT)
				...
				if (strncmp(nameofppid, "dumpstate", 9)) {
					oplus_root_check_succ(intuid_lst, inteuid_lst, intfsuid_lst, scno);
					oplus_root_reboot();
				}
