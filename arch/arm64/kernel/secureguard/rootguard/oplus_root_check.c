...
void oplus_root_check_succ(uid_t uid, uid_t euid, uid_t fsuid, uid_t callnum)
{
	...
	dcs_event->payload_length = snprintf(dcs_event_payload, 256,
	    "%d$$old_euid@@%d$$old_fsuid@@%d$$sys_call_number@@%d$$addr_limit@@%lx$$curr_uid@@%d$$curr_euid@@%d \
		$$curr_fsuid@@%d$$curr_name@@%s$$ppid@@%d$$ppidname@@%s$$enforce@@%d\n",
...
{
	/*Ke.Li@ROM, Security, 2019-12-16
	 *Root interception of the second phase of the optimization project:update the restart method,
	 *only kill the process of elevation
	 */
