...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	if (ilits->debug_gesture_sta) {
		if (ilits->ts->gesture_buf) {
			ipio_memcpy(ilits->ts->gesture_buf, buf, len,
	...
	if (!transfer) {
		if (ilits->position_high_resolution == OFF) {
			...
			gc->pos_start.x	= gc->pos_start.x * ilits->panel_wid /
					  TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_start.y = gc->pos_start.y * ilits->panel_hei /
					  TPD_HIGH_RESOLUTION_HEIGHT;
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	u16 self_key = 2;
	int ret = 0, tp_mode = ilits->actual_tp_mode, len = 0, geture_info_length = 0,
	    demo_mode_packet_len = 0;
...
int ili_report_handler(void)
{
	...
	if ((ilits->tr_buf[0] == P5_X_GESTURE_PACKET_ID)
			|| (ilits->tr_buf[0] == P5_X_GESTURE_FAIL_ID)) {
		if (ilits->position_high_resolution == OFF) {
				  ...
				  u8 read_type)
{
	...
	if (i < 10) {
		...
		for (i = 2 * xch * ych + offset_len; i < 2 * xch * ych + 2 * ych + offset_len;
				i += 2) {
			s16 temp;
		...
		for (i = 2 * xch * ych + 2 * ych + offset_len;
				i < 2 * xch * ych + 2 * ych + 2 * xch + offset_len; i += 2) {
			s16 temp;
			temp = (s16)((buf[i] << 8) + buf[i + 1]);
			seq_printf(s, "%5d,", temp);
		}
		...
		for (i = 2 * xch * ych + 2 * ych + 2 * xch + offset_len;
				i < ilits->tp_data_len - 1; i += 2) {
			s16 temp;
			temp = (s16)((buf[i] << 8) + buf[i + 1]);
			seq_printf(s, "%5d,", temp);
		}
