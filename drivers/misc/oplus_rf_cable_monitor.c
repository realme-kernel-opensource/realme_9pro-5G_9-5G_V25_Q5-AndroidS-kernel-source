...
static int rf_cable_initial_request_irq(struct rf_cable_data *rf_data)
{
	...
	for (i = 0; i < rf_data->rf_cable_support_num; i++) {
		if (gpio_is_valid(rf_data->cable_gpio[i])) {
			rc = devm_request_threaded_irq(rf_data->dev, rf_data->cable_irq[i], NULL,
						       cable_interrupt,
	...
	for (i = 0; i < rf_data->rf_pds_support_num; i++) {
		if (gpio_is_valid(rf_data->pds_gpio[i])) {
			rc = devm_request_threaded_irq(rf_data->dev, rf_data->pds_irq[i], NULL,
						       cable_interrupt,
						       IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_SUSPEND,
						       "rf_pds_irq", rf_data);
 ...
 struct rf_cable_data *rf_data)
{
	...
	rf_data->rf_cable_active = pinctrl_lookup_state(rf_data->gpio_pinctrl,
				   "rf_cable_active");
	rf_data->rf_pds_active = pinctrl_lookup_state(rf_data->gpio_pinctrl,
				 "rf_pds_active");
...
static int op_rf_cable_probe(struct platform_device *pdev)
{
	...
	rf_data->the_rf_format = (struct rf_info_type *)qcom_smem_get(SMEM_APPS,
				 SMEM_DRAM_TYPE, &smem_size);
