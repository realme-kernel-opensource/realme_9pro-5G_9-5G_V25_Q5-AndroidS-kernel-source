...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if(g_oplus_chip) {
		chg->cclogic_gpio = of_get_named_gpio(node, "qcom,cclogic-gpio", 0);
		if (chg->cclogic_gpio <= 0) {
			chg_err("Couldn't read qcom,cclogic-gpio\n");
		} else {
			rc = gpio_request(chg->cclogic_gpio, "cclogic-gpio");
			if (rc < 0) {
				chg_err("unable to request cclogic-gpio\n");
			} else {
				rc = gpio_direction_output(chg->cclogic_gpio, 1);
				if(rc < 0) {
					chg_err("unable to set cclogic-gpio output high\n");
				}
			}
		}
	}
