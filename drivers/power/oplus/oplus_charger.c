...
static int oplus_cclogic_gpio_init(struct oplus_chg_chip *chip)
{
	...
	if(chg->cclogic_gpio <= 0) {
		chg_err("Couldn't read qcom,cclogic-gpio\n");
		return -EINVAL;
	}
	level = gpio_get_value(chg->cclogic_gpio);
	chg_err("%s:cclogic_gpio:level:%d\n", __func__, level);

	if(level == 0) {
		gpio_set_value(chg->cclogic_gpio, 1);
		chg_err("%s:cclogic_gpio:need to set to high:level:%d\n",
				__func__, level);
	}
