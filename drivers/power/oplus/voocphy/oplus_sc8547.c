...
static int sc8547_read_byte(struct i2c_client *client, u8 reg, u8 *data)
{
	...
	if(atomic_read(&oplus_voocphy_mg->suspended) == 0) {
		mutex_lock(&i2c_rw_lock);
		ret = __sc8547_read_byte(client, reg, data);
		if(ret < 0) {
			while(retry > 0 && atomic_read(&oplus_voocphy_mg->suspended) == 0) {
...
static int __sc8547_update_bits(struct i2c_client *client, u8 reg, u8 mask, u8 data)
{
	int ret;
	u8 tmp;
                              ...
                              u8 mask, u8 data)
{
	...
	if(atomic_read(&oplus_voocphy_mg->suspended) == 0) {
		mutex_lock(&i2c_rw_lock);
		ret = __sc8547_update_bits(client, reg, mask, data);
		if(ret < 0) {
			while(retry > 0 && atomic_read(&oplus_voocphy_mg->suspended) == 0) {
				usleep_range(10000, 10000);
				ret = __sc8547_update_bits(client, reg, mask, data);
...
static s32 __sc8547_read_word(struct i2c_client *client, u8 reg)
{
	s32 ret;
...
static s32 sc8547_read_word(struct i2c_client *client, u8 reg)
{
	...
	if(atomic_read(&oplus_voocphy_mg->suspended) == 0) {
		mutex_lock(&i2c_rw_lock);
		ret = __sc8547_read_word(client, reg);
		if (ret < 0) {
			while(retry > 0 && atomic_read(&oplus_voocphy_mg->suspended) == 0) {
				usleep_range(10000, 10000);
				ret = __sc8547_read_word(client, reg);
...
static s32 __sc8547_write_word(struct i2c_client *client, u8 reg, u16 val)
{
	s32 ret;
...
static s32 sc8547_write_word(struct i2c_client *client, u8 reg, u16 val)
{
	...
	if(atomic_read(&oplus_voocphy_mg->suspended) == 0) {
		mutex_lock(&i2c_rw_lock);
		ret = __sc8547_write_word(client, reg, val);
		if (ret < 0) {
			while(retry > 0 && atomic_read(&oplus_voocphy_mg->suspended) == 0) {
				usleep_range(10000, 10000);
				ret = __sc8547_write_word(client, reg, val);
...
int sc8547_get_cp_vbus(struct oplus_voocphy_manager *chip)
{
	...
	chip->cp_vbus = ((data_block[0] << 8) | data_block[1]) * 375 / 100;
...
static int sc8547_vooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_09, 0x93);	//WD:5000ms
...
static int sc8547_5v2a_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	sc8547_write_byte(chip->client, SC8547_REG_07, 0x04);
	sc8547_write_byte(chip->client, SC8547_REG_09, 0x10);	//WD:
...
static int oplus_sc8547_suspend_noirq(struct device *device)
{
	/*pr_err("+++ prepare %s: enter +++\n", __func__);*/
	if (oplus_voocphy_mg) {
		atomic_set(&oplus_voocphy_mg->suspended, 1);
	} else {
		pr_err("%s: oplus_voocphy_mg is null\n", __func__);
	}
	/*pr_err("--- prepare %s: exit ---\n", __func__);*/
	return 0;
}

static void oplus_sc8547_resume_noirq(struct device *device)
{
	/*pr_err("+++ complete %s: enter +++\n", __func__);*/
	if (oplus_voocphy_mg) {
		atomic_set(&oplus_voocphy_mg->suspended, 0);
	} else {
		pr_err("%s: oplus_voocphy_mg is null\n", __func__);
	}
...
static const struct dev_pm_ops oplus_sc8547_pm_ops = {
	.prepare		= oplus_sc8547_suspend_noirq,
	.complete		= oplus_sc8547_resume_noirq,
};
