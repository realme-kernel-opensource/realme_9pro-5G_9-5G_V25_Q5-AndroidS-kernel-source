...
static int p922x_MTP(struct oplus_p922x_ic *chip, unsigned char *fw_buf, int fw_size)
{
	...
	rc = __p922x_write_reg(chip, 0x405,
			       (pure_fw_size >> 8) & 0xff); /* write FW length high byte */
...
static int p922x_wpc_get_adapter_power(void)
{
	struct oplus_p922x_ic *chip = p922x_chip;
	...
	return chip->p922x_chg_status.adapter_power;
}
...
struct oplus_wpc_operations p922x_ops = {
	.wpc_get_adapter_power = p922x_wpc_get_adapter_power,
