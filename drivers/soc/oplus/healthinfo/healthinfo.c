...
/*Default*/
static int ohm_ctrl_list = OHM_LIST_MAGIC | OHM_CTRL_CPU_CUR | OHM_CTRL_MEMMON |
			   OHM_CTRL_IONMON | OHM_CTRL_SCHEDTOTAL;
...
/******  Cur cpuloading  ******/
static ssize_t cpu_load_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	...
	len = sprintf(page,
		      "cur_cpuloading: %d\n""cur_cpu_ctrl: %s\n""cur_cpu_logon: %s\n""cur_cpu_trig: %s\n",
		      load, (ohm_cpu_ctrl ? "true" : "false"), (ohm_cpu_logon ? "true" : "false"),
			       ...
			       size_t count, loff_t *off)
{
	...
	len = sprintf(page,
		      ...
		      ohm_memmon_ctrl ? "true" : "false", ohm_memmon_logon ? "true" : "false",
...
static ssize_t ion_wait_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
	...
	len = sprintf(page, "total_ion_wait_h_cnt: %lld\n""total_ion_wait_l_cnt: %lld\n"
		      ...
		      ohm_ionmon_ctrl ? "true" : "false", ohm_ionmon_logon ? "true" : "false",
...
/******  Proc para   ******/
static ssize_t ohm_para_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	char page[BUFFER_SIZE_S] = {0};
	int len = 0;
	...
	len = sprintf(page,
		      "action: %s\n""ctrl: 0x%08x\n""logon: 0x%08x\n""trig: 0x%08x\n",
		      (ohm_action_ctrl ? "true" : "false"), ohm_ctrl_list, ohm_logon_list,
		      ohm_trig_list);
...
static ssize_t cpu_info_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	char page[BUFFER_SIZE_L] = {0};
	int len = 0;
	...
	for_each_possible_cpu(cpu) {
		...
		len += snprintf(page + len, sizeof(page) - len, "%u ",
...
static ssize_t ohm_thresh_write_common(struct file *file,
				       const char __user *buff, size_t len, loff_t *ppos, int type)
{
	char write_data[32] = {0};
...
#endif
#if IS_BUILTIN(CONFIG_OPLUS_FEATURE_ZRAM_OPT)
...
static int __init healthinfo_init(void)
{
	...
	pentry = proc_create("iowait_thresh", S_IRUGO | S_IWUGO, sched_thresh,
