...
static ssize_t mag_data_int2str(char *buf)
{
	...
	for (index_i = 0; index_i < SOURCE_NUM; index_i++) {
		for (index_j = 0; index_j < 9; index_j++) {
			symble = g_chip->s_vector[OPLUS_MAG].hw[index_i].feature.parameter[index_j * 2 +
					1] > 0 ? -1 : 1;
			len = sprintf(buf + pos, "%d",
				      g_chip->s_vector[OPLUS_MAG].hw[index_i].feature.parameter[index_j * 2] *
				      symble);
