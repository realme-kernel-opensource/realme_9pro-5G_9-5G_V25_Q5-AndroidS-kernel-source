...
static int cb_config_show(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < min(NR_CLUS_MAX, cb_pol_idx); ++i) {
		...
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				"clus %d (switch %d) first_cpu %d ctl %d\n", i, cb_switch, cb->first_cpu,
				cb->ctl_inited);
...
void cb_update(struct cpufreq_policy *pol, u64 time)
{
	 ...
	 */
	delta = (min_over_target_freq || isolated) \
	 ? 0 : time - cb->last_ts;
	...
	if (debug)
		pr_info("cpu %d update: ts now %llu last %llu last_update %llu delta %llu
			 update_d %llu cur %u acc %llu, cur_level %d min_over_target_freq %d isolated %d last_core_boost %d\n",
			cpu,
			NSEC_TO_MSEC(time),
