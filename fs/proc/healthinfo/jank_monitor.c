...
void update_jank_trace_info(struct task_struct *tsk, int trace_type,
			    unsigned int cpu, u64 delta)
{
...
static int proc_jank_trace_show(struct seq_file *m, void *v)
{
	...
	u64 d_time, s_time, ltt_time, big_time, rn_time, iow_time, binder_time,
	    futex_time;
	...
	seq_printf(m,
		   "BR:%llu LR:%llu RN:%llu D:%llu IOW:%llu S:%llu BD:%llu FT:%llu\n",
		   big_time / NSEC_PER_MSEC, ltt_time / NSEC_PER_MSEC, rn_time / NSEC_PER_MSEC,
		   d_time / NSEC_PER_MSEC, iow_time / NSEC_PER_MSEC,
		   s_time / NSEC_PER_MSEC, binder_time / NSEC_PER_MSEC,
		   futex_time / NSEC_PER_MSEC);
	put_task_struct(p);
	return 0;
}
