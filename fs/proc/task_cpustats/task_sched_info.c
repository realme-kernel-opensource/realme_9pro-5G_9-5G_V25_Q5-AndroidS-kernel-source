...
void update_wake_tid(struct task_struct *p, struct task_struct *cur,
		     unsigned int type)
{
	u64 cur_tid = cur->pid;
...
void update_freq_limit_info(struct cpufreq_policy *policy)
{
	...
	freq_limit_info.sched_info_two = freq_limit_info.sched_info_two |
					 (cur_tid << 48);
...
static ssize_t proc_pids_set_write(struct file *file, const char __user *buf,
...
static int sched_buffer_open(struct inode *inode, struct file *file)
{
	...
	data_size = (arr_write < arr_read) ? (BUFFER_SIZE + arr_write - arr_read) :
		    arr_write - arr_read;
	cur_read = arr_read;
	cur_write = arr_write;
...
static int proc_task_sched_info_enable_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_task_sched_info_enable_show, inode);
}

static ssize_t proc_task_sched_info_enable_write(struct file *file,
...
static int proc_sched_info_threshold_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_sched_info_threshold_show, inode);
}

static ssize_t proc_sched_info_threshold_write(struct file *file,
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	while ((single = strsep(&all, " ")) != NULL
			&& threshold_num < MAX_THRESHOLD_NUM) {
		target = single;
...
static int __init sched_info_init(void)
{
	...
	proc_entry = proc_create("pids_set", S_IRUGO | S_IWUGO, sched_info,
				 &proc_pids_set_operations);
	...
	proc_entry = proc_create("sched_buffer", S_IRUGO | S_IWUGO, sched_info,
				 &proc_sched_buffer_operations);
	...
	proc_entry = proc_create("task_sched_info_enable", S_IRUGO | S_IWUGO,
				 sched_info, &proc_task_sched_info_enable_operations);
	...
	proc_entry = proc_create("sched_info_threshold", S_IRUGO | S_IWUGO, sched_info,
				 &proc_sched_info_threshold_operations);
