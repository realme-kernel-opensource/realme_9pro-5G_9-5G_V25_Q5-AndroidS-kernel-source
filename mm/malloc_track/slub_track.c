...
static int kd_list_locations(struct kmem_cache *s, char *buf, int buff_len)
{
	...
	int node;
	unsigned long *map = vmalloc(BITS_TO_LONGS(oo_objects(s->max)) * sizeof(
					     unsigned long));
...
static ssize_t kmalloc_debug_create_write(struct file *file,
...
extern int kbuf_dump_kmalloc_debug(struct kmem_cache *s, char *kbuf,
				   int buff_len);
...
static void *kmd_start(struct seq_file *m, loff_t *pos)
{
	...
	while (*pos <= ALL_KMALLOC_HIGH) {
		s = (struct kmem_cache *)atomic64_read(&kmalloc_debug_caches[CACHE_TYPE(
				*pos)][CACHE_INDEX(*pos)]);
...
static int memleak_detect_thread(void *arg)
{
	...
	do {
		...
		for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
			for (i = 0; i <= KMALLOC_SHIFT_HIGH; i++) {
				struct kmem_cache *s = (struct kmem_cache *)atomic64_read(
							       &kmalloc_debug_caches[type][i]);
...
static ssize_t memleak_detect_thread_write(struct file *file,
		const char __user *buff,
		size_t len, loff_t *ppos)
{
	char kbuf[16] = {'0'};
