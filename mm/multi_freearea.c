	...
	for (zone_type = 0; zone_type < MAX_NR_ZONES; zone_type++) {
		...
		for (flc = 0; flc < FREE_AREA_COUNTS; flc++)
			seq_printf(m, "[%d]: label = %lu, segment = %lu\n", flc,
				   zone->zone_label[flc].label, zone->zone_label[flc].segment);
...
static ssize_t proc_free_area_write(struct file *file, const char __user *buff,
...
void list_sort_add(struct page *page, struct zone *zone, unsigned int order,
		   int mt)
{
...
void ajust_zone_label(struct zone *zone)
{
	...
	for (i = 0; i < FREE_AREA_COUNTS; i++)
		zone->zone_label[i].label = zone->zone_start_pfn + zone->spanned_pages *
					    (i + 1) / FREE_AREA_COUNTS;
