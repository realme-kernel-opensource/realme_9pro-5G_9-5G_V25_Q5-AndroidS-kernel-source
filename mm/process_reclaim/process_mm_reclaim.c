...
static noinline ssize_t reclaim_task_write(struct task_struct *task,
		char *buffer)
{
	struct mm_struct *mm;
	struct vm_area_struct *vma;
...
cont:
	...
	/* If not timeout and not reach the mmap end,
	 * continue.
	 */
	if (((err == PR_PASS) || (err == -PR_FULL)) && vma)
		goto cont;
...
static int __init process_reclaim_proc_init(void)
{
	...
	rc = register_trace_android_vh_check_process_reclaimer(
		     current_is_process_reclaimer, NULL);
	...
	if (rc) {
		unregister_trace_android_vh_check_process_reclaimer(
			current_is_process_reclaimer, NULL);
		return rc;
	}
