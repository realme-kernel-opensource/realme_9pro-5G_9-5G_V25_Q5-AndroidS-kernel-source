...
static int memory_monitor_show(struct seq_file *m, void *p)
{
	...
	seq_printf(m,
		   "%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-24s\t%-18s\n",
	...
	for (i = 0; i < record_tasks; i++) {
		...
		seq_printf(m,
			   "%-24s\t%-24d\t%-24u\t%-24hd\t%-24lu\t%-24lu\t%-24lu\t%-24lu\t%-24lu\t%-18d\n",
	...
	seq_printf(m,
		   "read time: %lu ns, rcu_lock: %lu, task_lock_max : %lu, gpu_max_read_time = %lu, record_tasks = %d\n",
...
static ssize_t memory_monitor_write(struct file *file, const char __user *buff,
