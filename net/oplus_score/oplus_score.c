...
static int oplus_update_score_count(int link, int score, int flag)
{
	...
	if (flag) {
		...
		else if (score < WORST_BASE_SCORE) {
			uplink_score_info[link].uplink_score_count--;
			if (uplink_score_info[link].uplink_score_count < -SCORE_KEEP) {
				uplink_score_info[link].uplink_score_count = -SCORE_KEEP;
			}
...
void oplus_score_calc_and_report(void)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:up_score:link=%u,up_pack=%u,up_retran=%u,up_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u\n",
				uplink_index, uplink_packets, uplink_retrans_packets, uplink_srtt, uplink_score,
				uplink_smooth_score, uplink_seq, oplus_score_foreground_uid[0], retrans_rate, index);
		}
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:down_score:link=%u,down_pack=%u,down_retran=%u,down_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u\n",
		...
		if (uplink_report || downlink_report) {
			...
			if (oplus_score_debug) {
				printk("[oplus_score]:report_score1:link=%u,up_score=%d,down_score=%d,uid=%u,ul_p=%d,dl_p=%d\n",
					uplink_index, link_score_msg.uplink_score, link_score_msg.downlink_score,
...
static void oplus_score_uplink_stat(struct sk_buff *skb)
{
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:link=%d,seq=%u,high_seq=%u,uplink_retran=%u,npacket=%u,uid=%u,sport=%u,dport=%u,state=%d,len=%u\n",
				link_index, ntohl(tcph->seq), tp->high_seq, uplink_score_info[i].uplink_retrans_packets,
...
static void oplus_score_downlink_stat(struct sk_buff *skb)
{
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:link=%d,seq=%u,rcv_nxt=%u,downlink_retran=%u,npacket=%u,uid=%u,sport=%u,dport=%u,state=%d,len=%u\n",
				link_index, ntohl(tcph->seq), tp->rcv_nxt, downlink_score_info[i].downlink_retrans_packets,
...
static void oplus_score_add_link(struct nlattr *nla)
{
	...
	printk("[oplus_score]:add link index=%u,dev_name=%s, uplink_num=%d,downlink_num=%d!\n",
...
static void oplus_score_del_link(struct nlattr *nla)
{
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:del link index=%u,uplink_num=%d,downlink_num=%d!\n",
