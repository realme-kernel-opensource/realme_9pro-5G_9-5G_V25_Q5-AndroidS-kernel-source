...
struct cam_sensor_i2c_reg_array fm24pc256e_wp_disable_setting[] = {
    {.reg_addr = 0x06CA, .reg_data = 0x0D, .delay = 0x00, .data_mask = 0x00}, \
};

struct cam_sensor_i2c_reg_array fm24pc256e_wp_enable_setting[] = {
    {.reg_addr = 0x06CA, .reg_data = 0x0F, .delay = 0x00, .data_mask = 0x00}, \
};
...
#define EEPROM_TYPE_GT24P128C2CSLI 0xFF
#define EEPROM_TYPE_FM24PC256E     0x01
#define EEPROM_TYPE_P24C256C       0x02
int32_t EEPROM_ControlWrite(struct cam_eeprom_ctrl_t *e_ctrl,
        struct cam_write_eeprom_t *cam_write_eeprom)
{
    int32_t  rc;
    uint32_t typeAddr = 0x0D;
    ...
    if (rc < 0) {
        ...
        CAM_ERR(CAM_EEPROM, "eeprom type: 0x%x", typeData);
    }

    if (typeData == EEPROM_TYPE_GT24P128C2CSLI) {
        rc = EEPROM_CommonWrite(e_ctrl, cam_write_eeprom);
        if ( rc < 0 ) {
            CAM_ERR(CAM_EEPROM, "EEPROM write gt24p128c2csli failed\n");
        }
    } else if (typeData == EEPROM_TYPE_FM24PC256E) {
        rc = EEPROM_Fm24pc256eWrite(e_ctrl, cam_write_eeprom);
        if ( rc < 0 ) {
            CAM_ERR(CAM_EEPROM, "EEPROM write Fm24pc256e failed\n");
        }
    } else if (typeData == EEPROM_TYPE_P24C256C) {
        ...
        struct cam_write_eeprom_t *cam_write_eeprom)
{
    ...
    e_ctrl->io_master_info.cci_client->sid = 0xB0 >> 1;
    ...
    if ( rc < 0 ) {
        ...
        m_eeprom_size = cam_write_eeprom->calibDataSize;
        e_ctrl->io_master_info.cci_client->sid = 0xA0 >> 1;
        CAM_ERR(CAM_EEPROM, "EEPROM fm24pc256e write success, calibDataSize:%d, m_eeprom_size:%d, base_addr: 0x%x",
            cam_write_eeprom->calibDataSize,
            m_eeprom_size,
...
int32_t EEPROM_P24c256cWrite(struct cam_eeprom_ctrl_t *e_ctrl,
        struct cam_write_eeprom_t *cam_write_eeprom)
{
    int i = 0;
    int j = 0;
    ...
    void *arg)
{
    ...
    if (strncmp(cam_write_eeprom.eepromName, "imx766_fm24pc256e_gt24p128c2csli", 20) == 0) {
        is_fm24pc256e = true;
    } else if (e_ctrl->io_master_info.cci_client->sid == (0x68 >> 1)) {
        is_sem1215s = true;
